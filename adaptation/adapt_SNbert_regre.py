"""
Fine-tune ChemBERTa + RDKit descriptor fusion for molecular regression (MSE loss).
...
def test_model(model, test_loader, criterion, device, use_amp=False):
    """Evaluate on test set (regression): predictions, labels, MSE, MAE, R2."""
    model.eval()
    total_loss = 0.0
    all_predictions = []
    all_labels = []
    ...
    with torch.no_grad():
        for batch in tqdm(test_loader, desc="Test"):
            ...
            total_loss += loss.item()
            all_predictions.append(logits.detach().cpu().float().numpy())
...
def parse_args(argv: Optional[list] = None):
    ...
    p = argparse.ArgumentParser(
        description='Fine-tune ChemBERTa + descriptor fusion for regression (MSE loss).',
        parents=[pre],
    )
    ...
    p.add_argument('--num-classes', type=int, default=g('num_classes', 1),
                   help='Output dim: 1 for regression, >1 for classification')
...
def main():
    ...
    print("=" * 50)
    print(f"Test loss (MSE): {test_results['loss']:.6f}")
